...
[NOTE]
*Note:* To create a subscription, you must first have at least one plan defined in the catalog.
...
[start=2]
. Fill in the fields. For field information, see <<>>.
. Click the *Save* button.
...
. To change the default cancellation policy for the plan:
** To switch to `IMMEDIATE` or `END_OF_TERM`, select the *Specify Policy* radio button and choose `IMMEDIATE` or `END_OF_TERM` from the *Policy* field:

image::ChangePlan-SpecifyPolicy.png[width=350,align="center"]

*** To select a specific cancellation date, select the *Specify a date* radio button and choose a date from the *Change Date* calendar field.
...
1. Open the account and click *Subscriptions* on the sub-menu.
2. In the Actions column (far right), click *Cancel*. Kaui displays several options. The option you select defines how Kill Bill handles billing for the canceled subscription:

image::Subscriptions-CancelOptions.png[width=350,align="center"]

===== Cancellation Options Descriptions
...
| Cancels the subscription immediately. Kill Bill refunds whatever amounts have been paid toward the subscription.
...
Follow the steps below to update the billing cycle day (BCD) defined for the customer subscription(s), assuming the subscription is billed on a monthly basis.

*Important!* The new BCD must be in the future so as to not modify existing invoices.
...
image::UpdateSubscriptionBCD.png[width=350,align="center"]
