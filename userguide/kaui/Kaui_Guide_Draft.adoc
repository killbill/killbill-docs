...
* To switch to `IMMEDIATE` or `END_OF_TERM`, select the *Specify Policy* option and choose one from the *Policy* field:

image::ChangePlan-SpecifyPolicy.png[width=350,align="center"]

* To select a specific cancellation date, select the *Specify a date* option and choose a date from the *Change Date* calendar field.
...
*Important!* The new BCD must be in the future so as to not modify existing invoices. <-- NOT SURE ABOUT THIS
