...
//RESOURCES
//Mary's outline: https://docs.google.com/document/d/13AWO6ubgPKSA3Ne_nbW9vmmmBm1s9AB27q8jqG74NbQ/edit?usp=sharing <-- *** Include this in pull requests!***
...
This area of Kaui lists the invoices applied to the account.

[[more description of screen and features]]

From this screen, you can select an invoice and make a payment on it ...

==== About Automatic Invoice Payments

Kill Bill generates an invoice automatically if:

* An account has an active, recurring subscription _and_
* An account has a default and valid payment method _or_ you add a charge to the account.
...
MORE ON THIS? --> An example of this would be a check payment from the customer.
...
[start=3]
. Click on the number of the invoice that needs to be paid off. This opens the invoice detail.
...
|External
|Select this checkbox if the customer is making a payment outside of the Kill Bill system (such as with a check).

|Amount
|The amount defaults from the invoice balance. You can change this amount if the customer is making a partial payment.

|Payment Method
|To use the account’s default payment method, leave the field blank. For external payments, select `EXTERNAL_PAYMENT`.

|Reason and Comment
|The text you enter here displays on the <<Timeline>> page. Adding comments is optional.
