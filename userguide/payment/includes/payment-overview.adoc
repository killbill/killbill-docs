...
Kill Bill provides many open-source https://docs.killbill.io/latest/plugin_introduction.html#payment_gateways[payment plugins] corresponding to different payment gateways. In addition, Kill Bill also has a https://github.com/killbill/killbill-plugin-api[Plugin API] which can be used for developing a custom payment plugin corresponding to a specific payment gateway as explained in the https://docs.killbill.io/latest/payment_plugin.html[_Plugin Development Guide_].
