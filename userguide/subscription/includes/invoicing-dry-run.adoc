...
Finally, we 'll generate a dry-run invoice associated to a change in the subscription: We want to see what kind of invoice would be generated if we were to change to billing period from `ANNUAL` to `MONTHLY`. Also note that this changed was made on 2018-01-24:
