...
* *BillCycleDay (BCD)* - The day in the month where the invoicing for a particular subscription happens (this only makes sense for recurring subscriptions whose `Billing Period` is a multiple of the month). For other schemes (e.g. `WEEKLY` the day will vary for each invoice), in which case there is no `BCD` defined. 
...
* **Plan Cancellation Timing** - Should the customer gets a pro-ration credit when the subscription is cancelled before its next renewal date?
