...
4. Ensure that you have http://curl.haxx.se/[cURL] installed. This is only to be able to run the setup steps and examples from this documentation. In practice, your application will use one of our https://apidocs.killbill.io/[client libraries]. If you are on Windows, we recommend that you use https://git-scm.com/download/win[Git Bash] to run the `cURL` commands.
