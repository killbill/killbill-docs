...
Aggregate functions are supported for metrics, for example (*%20* is the URL encoded representation of a space): report1=payments_per_day\^dimension:currency^dimension:state\^metric:sum(amount)^metric:count(distinct%20fee)
