...
Aggregate functions are supported for metrics, for example (*%20* is the URL encoded representation of a space): report1=payments_per_day\^dimension:currency^dimension:state\^metric:sum(amount)^metric:count(distinct%20fee)^metric:100*sum(fee)/amount
...
You can also setup aliases and/or skip *Other* as such: report1=payments_per_day\^dimension:currency(USD=Main%20currency|EUR=Second%20currency|BRL,GBP,MXN,AUD=Third%20currency|-)^dimension:state\^metric:amount^metric:fee
