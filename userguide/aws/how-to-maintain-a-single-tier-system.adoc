...
2. Delete the cached directory `/var/lib/tomcat/webapps/ROOT`
3. Run the command `$KPM_INSTALL_KB_CMD`
4. Restart KB using `sudo service killbill restart`.
...
The KB single tier AMI comes pre-configured with a self-signed certificate. Some backend systems might not trust this certificate, leading to SSL related failures. There are two possible solutions:

1. You can update the backend to trust KB's self-signed certificate. This involves configuring the backend application to trust the certificate. For instance, if your backend is using Java, we can add KB's certificate to the truststore. The certificate used by KB can be found at `/etc/nginx/ssl/ssl-cert-snakeoil.pem` (public key) and `/etc/nginx/ssl/ssl-cert-snakeoil.key` (private key).
