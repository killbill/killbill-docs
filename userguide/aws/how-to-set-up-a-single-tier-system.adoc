...
Your security group should now have three inbound rules: `443`, `8443`, and `22`, all open to `0.0.0.0/0`.
...
You can now try to login to Kaui from your browser using the URL `\https://INSTANCE_IP:443` (or simply `\https://INSTANCE_IP`), where INSTANCE_IP is the IPV4 address for your instance, given on your dashboard as *Public IPV4 Address*. If all goes well, this should display the Kaui login screen. The browser may complain that your connection is not secure, but it should offer you a way to bypass the problem.
