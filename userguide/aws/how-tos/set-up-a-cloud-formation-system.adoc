...
Your deployment is initially set up to accept external communications, such as Kaui interaction, using HTTP. For secure and private communication it is necessary to switch to HTTPS. this requires an X.509 certificate that you must provide. If you do not have a certificate, you will create one using the AWS Certificate Manager.
...
Select the load balancer and click the Listeners tab. There are two listeners. These are configured by default to use HTTP on ports 9090 (Kaui) and 80 (Kill Bill). We will change these to use HTTPS.

Select the listener for port 9090. This is the Kaui listener. Under the Actions dropdown choose **Edit Listener**.

image::../../assets/aws/cf-edit-listener.png[align=center]

Change HTTP to HTTPS. The port is automatically set to 443. A new section will open at the bottom of the page, called **Secure Listener Settings**.

image::../../assets/aws/cf-secure-listener-settings.png[align=center]

If you do not have a certificate, click **Request a certificate from ACM**, then follow the instructions in https://add-a-certificate-using-ACM.html[How to Add a Certificate Using ACM].

If you have a certificate, not created by ACM, set the bottom left dropdown to **Import** and follow the instructions. Import the certificate to ACM.

If you have a certificate in ACM, including one you just imported or created, set the bottom left dropdown to **From ACM** and select your certificate in the bottom right dropdown.

Finally, at the bottom select **Save changes**.

Similarly, edit the second listener and change HTTP to HTTPS. Type 8443 as the port number. Be sure your certificate is selected at the bottom, and click **Save Changes**.

Now return to the Listener tab on the Load Balancer page. You will see:

image::../../assets/aws/cf-listeners-not-reachable.png[align=center]

What happened? The (now secure) listeners are not reachable because we have not yet enabled HTTPS in their security group.

To fix this, return to the Load Balancer page and select the *Security* tab. Click on the security group name. Make sure the *Inbound rules* tab is selected, then click *Edit inbound rules*.

Change the rule with Type HTTP to Type HTTPS. The Port Range automatically becomes 443.

For the rule with Type Custom TCP, set the Port Range to 8443.

You should now have:

image::../../assets/aws/cf-new-inboud-rules.png[align=center]


Click *Save rules*, then return to the Load Balancer page. The "Not Reachable" warnings should be gone.
...
For an introduction to Kaui, see our https://docs.killbill.io/latest/quick_start_with_kaui.html[Quick Start with Kaui] guide. The default credentials are: `admin` / `<INSTANCE_ID>`, where <INSTANCE_ID> is the AWS ID for the same instance you selected for login.

Similarly, you should be able to login directly to the Kill Bill server using the URL `\https://kaui.<DOMAIN>:8443`.
This provides access to certain detailed reports that may be needed for maintenance, including metrics, event logs, and the Swagger API pages.
