...
This document explains how to set up Kill Bill on AWS using the multi-tier option. This option is a recommended alternative for production environments. The multi-tier option requires more setup than CloudFormation, but provides more control over the deployment.
...
image::../../assets/aws/multi-tier-ami_deployment.png[width=750,align=center]
