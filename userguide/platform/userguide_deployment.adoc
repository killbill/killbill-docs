...
Download the current stable version of the Kill Bill war from https://search.maven.org/search?q=a:killbill-profiles-killbill[Maven Central] and place it under *CATALINA_HOME/webapps/ROOT.war* (delete the directory *CATALINA_HOME/webapps/ROOT* if it exists).
...
Follow similar steps to install Kaui (also available from https://search.maven.org/search?q=a:kaui-standalone[Maven Central]).

Note: unlike other installation methods, you will also need to install and configure your database manually. See below for more details.
