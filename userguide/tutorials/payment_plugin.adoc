...
As explained earlier, the Kill Bill Janitor system attempts to fix *PENDING* and *UNKNOWN* states by  polling the plugin via `PaymentPluginApi#getPaymentInfo` method. So, this method needs to be implemented as follows:
[source,java]
	public List<PaymentTransactionInfoPlugin> getPaymentInfo(UUID kbAccountId, UUID kbPaymentId,
		...
		String paymentId = PluginProperties.findPluginPropertyValue("payment_id", properties);
		Payment payment = client.payments().get(paymentId).execute();
...
* Like the other methods, the `getPaymentInfo` method accepts  `Iterable<PluginProperty> properties` as a parameter.  

* The code first retrieves the `paymentId`, sent by the Janitor system from the properties passed in. 

* Next, the `payments().get()` is invoked with the `paymentId`. This returns a `Payment` object with additional details about the payment as specified https://developer.gocardless.com/api-reference/#payments-get-a-single-payment[here].

* The code then retrieves the GoCardless currency(object of type `com.gocardless.resources.Payment.Currency`) from the `payment` object and converts it to Kill Bill currency (object of type `org.killbill.billing.catalog.api.Currency`).

* Next, the code retrieves the GoCardless status (object of type `com.gocardless.resources.Payment.Status`) from the `payment` object and converts it to a `org.killbill.billing.payment.plugin.api.PaymentPluginStatus` object.

* Finally, it builds the `GoCardlessPaymentTransactionInfoPlugin` object using these parameters, adds it to a List and returns it.

* Depending on the status in the `GoCardlessPaymentTransactionInfoPlugin` object, the Janitor updates the internal payment state as well as invoice balance, etc.
