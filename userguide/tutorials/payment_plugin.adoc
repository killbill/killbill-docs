...
*Checkout Servlet* - Checkout servlet that initiates setting up the payment method. This is explained https://docs.killbill.io/latest/payment_plugin.html#_step_9_creating_gocardlesscheckoutservlet[further] in detail
...
. The merchant site invokes the `GoCardlessCheckoutServlet`.

. The `GoCardlessCheckoutServlet` invokes the `GoCarldessPlugin#buildFormDescriptor`.
...
* This invokes the `KillBill#addPaymentMethod` which in turn invokes `GoCardlessPlugin#addPaymentMethod`.
