...
. Create a new Maven project similar to the *Hello World Plugin* 
...
The following table lists the methods in https://github.com/killbill/killbill-plugin-api/blob/master/payment/src/main/java/org/killbill/billing/payment/plugin/api/PaymentPluginApi.java[PaymentPluginApi] interface. You can implement the required methods in your class.
...
It is very important to correctly populate the status type in the https://github.com/killbill/killbill-api/blob/4ae1c343a593de937415e21feecb9f5405037fa3/src/main/java/org/killbill/billing/payment/plugin/api/PaymentPluginStatus.java[PaymentPluginStatus].   The following table elaborates how the status should be populated:
...
GoCardless provides a https://developer.gocardless.com/getting-started/api/making-your-first-request/#setting-up-your-client-library[client library]. We will be using this library to integrate GoCardless with Kill Bill. For the sake of simplicity, we will be creating a very basic plugin that can only process payments. 
...
The first step in GoCardless would be adding a customer and setting up the payment mandate. GoCardless supports three ways to do this as explained https://developer.gocardless.com/getting-started/api/adding-your-first-customer/[here]. In this tutorial, we will be using the *hosted payment page* approach.  This includes the following steps as explained https://developer.gocardless.com/getting-started/api/adding-your-first-customer/#setting-up-direct-debit-mandates-using-hosted-payment-pages[here]:
...
. The `GoCarldessPlugin#buildFormDescriptor` method initiates the GoCardless redirect flow as explained https://developer.gocardless.com/getting-started/api/adding-your-first-customer/[here] and obtains the *redirect URL* which is returned to the client application
...
. The `GoCarldessPlugin#addPaymentMethod` completes the redirect flow as explained https://developer.gocardless.com/getting-started/api/adding-your-first-customer/[here] and obtains the *mandate id* which is saved in the Kill Bill database
...
. Add the following additional Maven dependency:
[source,bash]
        <dependency>
...
** `client` of type http://gocardless.github.io/gocardless-pro-java/com/gocardless/GoCardlessClient.html[GoCardlessClient]. This is a GoCardless specific class that can be used to access the GoCardless API
...
* The `GoCardlessPaymentTransactionInfoPlugin` extends the https://github.com/killbill/killbill-plugin-framework-java/blob/46d94fbeb1cf089aa04e62cfecf751ca47032023/src/main/java/org/killbill/billing/plugin/api/payment/PluginPaymentTransactionInfoPlugin.java[PluginPaymentTransactionInfoPlugin] class from the Kill Bill plugin framework.

* The `GoCardlessPaymentTransactionInfoPlugin` constructor simply invokes the superclass constructor.
...
As explained earlier, the `buildFormDescriptor` method needs to be implemented to *initiate* the *Gocardless redirect flow* in GoCardless and obtain the *redirect URL*. This method can be implemented as follows:
...
* The code first retrieves the `successRedirectUrl`, `redirectFlowDescription` and `sessionToken` sent by the client application from the `properties` passed in. As explaind earlier, a client application needs to send a `successRedirectUrl`. The  `redirectFlowDescription` and `sessionToken` are GoCardless specific values, so the `buildFormDescriptor` method passes these values to the corresponding GoCardless method

* Next, the `client.redirectFlows().create()` is invoked which returns  a `RedirectFlow` object. The `RedirectFlow` object contains the *redirect URL*. 
...
The addPayment method is typically used to add a payment method in Kill Bill corresponding to a customer/account. In the case of GoCardless, we will be using it to *complete* the *redirect flow*. So, this method can be implemented as follows:
...
* `PaymentMethodPlugin` is a generic object that represents a payment method (creditcard, bank account, etc.). It has a `getProperties` method. 
...
* It then retrieves the `redirectFlowId` and `sessionToken` from `allProperties`. AIf you recall, the `redirectFlowId` is sent to a client application after the `buildFormDescriptor` method call. A client application needs to send this back. The  `sessionToken` is a GoCardless specific value, so the `addPaymentMethod` method just passes it to the corresponding GoCardless method
...
Once a user sets up a payment method as done above, the `purchasePayment` method can be used by a client application to trigger payments against a *mandateId*. So, this method can be implemented as follows:
...
* Like the other methods, the `purchasePayment` method accepts  `Iterable<PluginProperty> properties` as a parameter. This can be used to pass properties which are related to the `purchasePayment` call.
...
* Next, the `idempotencyKey` is retrieved from the `properties` passed in. The  `idempotencyKey` is a GoCardless specific value, so the `purchasePayment` method passes it to the corresponding GoCardless method
...
* Finally, the `client.payments().create()` is invoked with the `amount` and `currency` values. This returns a `Payment` object which contains a `paymentId`

* The `purchasePayment` method returns a `PaymentTransactionInfoPlugin` object.  The <<PaymentTransactionInfoPlugin>> section above explaind this interface in detail. In this case, we have created a `GoCardlessPaymentTransactionInfoPlugin` class. 
...
** *kbTransactionId* - Set to `kbTransactionId`. Tt corresponds to the Kill Bill transaction id. 
...
** - *PaymentPluginStatus* - Set to `PaymentPluginStatus.PROCESSED` since the payment was processed successfully 
...
** *secondPaymentReferenceId* - set to `null` since GoCardless does not have a secondPaymentReferenceId

** *createdDate* - Set to the current date

** *effectiveDate* - Set to the date when the payment was created 

** *properties* - Set to a `outputProperties`. This is a `List<PluginProperty>`  The GoCardless *payment Id* is added to this List. The client API can use these properties as desired.
...
** PaymentPluginStatus - Set to `PaymentPluginStatus.ERROR` since there was an error in the payment
...
* If the code is unable to retrieve the `mandateId` from the Kill Bill database, this object is created with the following values:
...
* It then invokes the `registerPaymentPluginApi` method which registers the servlet. This code is pretty standard across all plugins and can be used as it is
...
The GoCardless plugin can be built and deployed as per the instructions specified https://docs.killbill.io/latest/plugin_development.html#_development[here]
