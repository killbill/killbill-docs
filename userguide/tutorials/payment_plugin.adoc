...
* The code then iterates through the `payment` objects and obtains the `Payment` object corresponding to the `kbPaymentId` passed in. If you recall, the `purchasePayment` method sends `kbPaymentId` and `kbTransactionId` as metadata fields in GoCardless. Thus, the `kbPaymentId` is retrieved from the metadata of each `Payment` object and compared with the `kbPaymentId` passed in. 

* The code then creates a `GoCardlessPaymentTransactionInfoPlugin` corresponding to a matching `Payment` as follows:

** It first retrieves the *GoCardless currency* (`com.gocardless.resources.Payment.Currency`) from the `payment` object and converts it to *Kill Bill currency* ( `org.killbill.billing.catalog.api.Currency`).

** Next, the code retrieves the *GoCardless status* (`com.gocardless.resources.Payment.Status`) from the `payment` object and converts it to a *Kill Bill status* (`org.killbill.billing.payment.plugin.api.PaymentPluginStatus`).

** It then retrieves the `kbTransactionId` from the payment metadata. This step is very important because, the Janitor uses the *transaction id* to match the internal transactions with the plugin transactions as explained earlier. 

** Next, it creates some `PluginProperty` objects and adds them to the `outputProperties` List. Any arbitrary key-value pairs which need to be returned to the caller can be specified here. 

** Finally, it builds the `GoCardlessPaymentTransactionInfoPlugin` object using these parameters.
