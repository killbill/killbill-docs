...
** `client` This is of type http://gocardless.github.io/gocardless-pro-java/com/gocardless/GoCardlessClient.html[_GoCardlessClient_]. This is a GoCardless specific class that can be used to access the GoCardless API.
...
As explained earlier, most of the `PaymentPluginApi` methods return a `PaymentTransactionInfoPlugin` object (Refer to the <<PaymentTransactionInfoPlugin, "PaymentTransactionInfoPlugin">> section). Let us take a look at the https://github.com/killbill/killbill-gocardless-example-plugin/blob/9522498ecde5849c940574c598ceb5ce088d32a7/src/main/java/org/killbill/billing/plugin/gocardless/GoCardlessPaymentTransactionInfoPlugin.java[_GoCardlessPaymentTransactionInfoPlugin.java_] class. 
...
* An automatic payment is triggered by Kill Bill against an https://docs.killbill.io/latest/Kill-Bill-Glossary.html#invoice[_invoice_].
...
* The `createSession` accepts properties corresponding to `clock` and `goCardlessPaymentPluginApi`. These are injected via the `GoCardlessActivator` class (This class is explained in the <<gocardless_activator, "GoCardlessActivator">> section below).  
