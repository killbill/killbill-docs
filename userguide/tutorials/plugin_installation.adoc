...
* A `plugins` folder: This is where your plugin or 3rd party plugins deployed. This folder contains:
...
As explained in link:#_overview[overview section], Kill Bill have 2 locations to deploying plugins:
...
1. Create new folder with the name that represents your plugin. It is a good practice to set plugin name with "-plugin" suffix.
2. Add sub-folder that represent version of your plugin. For example, you can set `1.0.0` if it is brand-new plugin.
3. Put your plugin JAR file into folder you've created in point 2. It is recommended that your plugin have this following format: `<plugin-name>-<semver-plugin-version>.jar`.
