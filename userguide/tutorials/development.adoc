...
1. Create a killbill user and database (verify connectivity with `psql -h 127.0.0.1 -U killbill -p 5432`):

```
root=# create USER killbill;
root=# \password killbill;
root=# CREATE DATABASE killbill WITH OWNER killbill;
```

2. Create the schema: `./bin/db-helper -a create  --driver postgres -u killbill -p killbill `
3. Modify the `killbill.properties` under `profiles/killbill/src/main/resources`:
