...
Note that cloning the repository as specified above gives a read-only access to the repository. If you'd like to make changes to the repository, you need to fork the repository, make changes to your fork and create a Pull Request in order to merge the changes.
...
In order to configure MySQL, you need to follow the steps given below:
...
. Select the `killbill` database created above using the following command:

 use killbill;
 
. Run the Kill Bill DDL https://docs.killbill.io/latest/ddl.sql[here].

. Grant  privileges to the user created above on the killbill database using the following command:
