...
* Customers interact with your custom e-commerce website, which then issues REST API calls to Kill Bill. You are responsible for designing and building that piece.
...
A typical scenario is one where a subscription gets created, eventually leading to the creation of an _invoice_ and a _payment_. Reusing our example from above, i.e a deployment with the 2 plugins, the following would occur:
...
2. Because of its plugin architecture, it is easy to extend the system by writing plugins. The diagrams above assume the plugins interact with 3rd party systems, but of course all the tax logic could for instnace be implemented in the plugin itself.
