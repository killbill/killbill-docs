...
The CloudFormation template allows to specify different groups of subnets to isolate the Kill Bill server and KAUI server instances with the RDS instances. Typically one would specify at least 2 subnets, in different availability zones for each of those groups. Example: Given a VPC with a CIDR of `192.168.0.0/16`, one could create the following subnets:
...
**Pre-requisites: The CloudFormation template will require a valid VPC along with valid subnets to correctly create the stack**
