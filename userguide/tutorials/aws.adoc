...
Similarly one can `start`, `stop` the services using simalr command -- e.g `service kaui stop` to stop KAUI.
...
In order to access the database, one can use the following command  `mysql -u root -proot`. There is one `killbill` and one `kaui` database
created and used by the respective application
