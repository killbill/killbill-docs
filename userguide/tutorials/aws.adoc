...
We recomment to rely on the CloudFormation `ChangeSet` functionality to get a sense of what would be updated if the change was submitted. Here are some steps below:
...
[6]. Wait for the new stack to be up

After cloud formation change is done, you will see the `STATUS` or your stack transition back to `CREATE_COMPLETE`.
