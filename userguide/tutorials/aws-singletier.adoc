...
The easiest option to add the certificate is to make use of the tool `certbot`, which relies on the free Certificate Authority (CA) `Let’s Encrypt`. This method provides an easy way to obtain and install free certificates. For information on creating a certificate using this method, see https://docs.killbill.io/latest/using-https.html[Using HttPS].
