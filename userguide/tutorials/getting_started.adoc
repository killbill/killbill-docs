...
. Download the Tomcat container from http://tomcat.apache.org/download-80.cgi[here] (get the Core binary distribution). 

. Install Tomcat as per the instructions specified http://tomcat.apache.org/tomcat-8.0-doc/index.html[here].

. The rest of the guide assumes that you have installed Tomcat in a directory called *CATALINA_HOME*.


==== Tomcat configuration

. Download the Recommended Tomcat configuration files from  https://github.com/killbill/killbill-cloud/tree/master/ansible/templates/tomcat/conf[here]. The configuration files have the `.j2` extension for the Jinja templating language. 

. Render the files  using your custom variables (take a look at an example of such variables https://github.com/killbill/killbill-cloud/blob/master/ansible/group_vars/all.yml[here]). The custom variables depend on your environment.  https://ansible.sivel.net/test/[This website] can be used to perform the rendering online if you do not have Jinja installed.

. Place the `setenv2.sh` in *CATALINA_HOME/bin* 

. Plase the other files in *CATALINA_HOME/conf*.
...
. Add the Kill Bill configuration properties at the end of the *CATALINA_HOME/conf/catalina.properties* file.  At the  minimum, the  JDBC  parameters need to be added. So, for example, you can add the following(update the values as needed):
...
Unlike other installation methods, you will also need to install and configure your database manually. Configure the database as per the  https://docs.killbill.io/latest/userguide_deployment.html#_database_engine[deployment guide] here
