...
Users familiar with Java technologies can also install Kill Bill directly in the Web container of their choice. We recommend using Tomcat version 8.5. This section covers the instructions for installing killbill in Tomcat. If this approach is followed, you will also need to configure the database manually as explained below.

==== Tomcat Installation and configuration

The first step in installing Killbill in Tomcat is to download and install Tomcat. For this, you need to follow the steps given below:

. Download the Tomcat container from http://tomcat.apache.org/download-80.cgi[here] (get the Core binary distribution). The downloaded zip file contains a folder called *apache-tomcat-8.5.59*
...
. Open a browser and type http://localhost:8080/. If Tomcat is installed properly, this should display the following:
...
As mentioned above, you need to configure the database manually. For this, you need to follow the steps given below:
...
. If you are using PostgreSQL, run https://github.com/killbill/killbill/blob/master/util/src/main/resources/org/killbill/billing/util/ddl-postgresql.sql[this] schema extension DDL first

. Create a database. In MySQL, you can run the following commands to create a database called *killbill*: 
...
. You should have a database called *killbill* (Or whatever name you have specified in step 3) with the following tables:
...
In order to configure Killbill, you need to follow the steps given below:
...
. Verify that there are no errors in the Tomcat logs at  *TOMCAT_HOME/logs/catalina.out*.

. Verify that there are no errors in the Killbill logs on the console and that the logs display a line which states that  *Kill Bill server has started*

. Open a browser and type http://localhost:8080/. This should display the following:
...
==== Customizing Log file Path - TODO

Using the configuration above, the killbill logs get displayed on the standard output. You can however customise this so that they are saved to a separate log file. In order to set this up, you need to follow the steps given below:

. Open TOMCAT_HOME/conf/catalina.properties file.Add the following property related to the path of the logging configuration file:
  
  logback.configurationFile=/var/lib/killbill/config/logback.xml
  
. Download the logging configuration file from https://github.com/killbill/killbill-cloud/blob/master/ansible/templates/killbill/logback.xml.j2[here] 
. Place it in the path specified above `/var/lib/killbill/config/logback.xml` (refer to the Jinja templating language instructions above).
