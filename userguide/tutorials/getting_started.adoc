...
Users familiar with Java technologies can also install Kill Bill directly in the Web container of their choice, such as Tomcat. We recommend using Tomcat version 8.5.

==== Tomcat Installation

1. Download the Tomcat container from http://tomcat.apache.org/download-80.cgi[here] (get the Core binary distribution). 

1. Install Tomcat as per the instructions specified http://tomcat.apache.org/tomcat-8.0-doc/index.html[here].

1. The rest of the guide assumes that you have installed Tomcat in a directory called *CATALINA_HOME*.


==== Tomcat configuration

1. Download the Recommended Tomcat configuration files from  https://github.com/killbill/killbill-cloud/tree/master/ansible/templates/tomcat/conf[here].The configuration files have the `.j2` extension for the Jinja templating language. 

1. Render the files  using your custom variables (take a look at an example of such variables https://github.com/killbill/killbill-cloud/blob/master/ansible/group_vars/all.yml[here]). The , custom variables depend on your environment.  https://ansible.sivel.net/test/[This website] can be used to perform the rendering online if you do not have Jinja installed.

1. Place the `setenv2.sh` in *CATALINA_HOME/bin* 

1. Plase the other files in *CATALINA_HOME/conf*.
...
1. Add the Kill Bill configuration properties at the end of the *CATALINA_HOME/conf/catalina.properties* file.  At the  minimum, the  JDBC  parameters need to be added. So, for example, you can add the following(update the values as needed):
...
1. Place the logging configuration file https://github.com/killbill/killbill-cloud/blob/master/ansible/templates/killbill/logback.xml.j2[logback.xml] as `/var/lib/killbill/config/logback.xml` (refer to the Jinja templating language instructions above).
...
Unlike other installation methods, you will also need to install and configure your database manually. 

1. Configure the database as per the  https://docs.killbill.io/latest/userguide_deployment.html#_database_engine[deployment guide] here

==== Other Notes
1. Installation on Windows might work, but isn't tested nor recommended by the core team.
1. We recommend installing the https://tomcat.apache.org/native-doc/[Apache Tomcat Native Library] (automatically done for you when using Ansible, Docker, or in our Cloud Marketplace instances).
