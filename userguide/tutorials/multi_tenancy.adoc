...
In order to install Kill Bill on AWS you can use https://github.com/killbill/killbill-cloud/tree/master/docker[our docker] images and rely on the `aws` from `docker-machine`. Alternatively you can setup things by hand, for instance by following https://blog.axcess.io/2016/10/24/installing-killbill-on-aws-amazon-linux-based-ec2/[the following steps].
...
KAUI has been enhanced with new `admin screens`, that are described in the Multi-tenancy screens section https://github.com/killbill/killbill-admin-ui[of that documentation].
