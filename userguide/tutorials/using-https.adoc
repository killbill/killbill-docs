...
The AWS Certificate Manager (ACM) offers a straightforward way to provide SSL/TLS protection for AWS resources such as load balancers. This is most appropriate when using the multitier or cloud formation installation options. There is no extra cost for using ACM certificates. 
...
Create a CNAME, following instructions from your domain provider, as discussed above.
