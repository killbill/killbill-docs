...
At the time of login, the user will visit the Kaui login page and enter the username and password created in the authenticating system. Since Kaui is already aware of the username, a request will be sent to KillBill, which will then redirect the authentication request to the authenticating system. Upon receiving a successful response from Okta, Kaui will log in the user.
