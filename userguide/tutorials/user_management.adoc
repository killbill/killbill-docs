...
To enable LDAP, Kill Bill needs to be launched with the following System Properties (note that the values here are just examples and might need to be updated depending on your LDAP configuration):
...
* If you have trouble connecting to your LDAP system, enable DEBUG logging off the package `org.apache.shiro` in your `logback.xml` and check the logs
