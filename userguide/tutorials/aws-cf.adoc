...
For temporary use, you can sometimes create a *self-signed* certificate, in which you are your own CA. Most browsers will frown on this, since you are simply vouching for yourself, but it allows you to get started with security.
...
To create a self-signed certificate, use the following commands in your local environment:
...
There are a couple of possible issues when starting the stack from CloudFormation (CFN). The goal of this section
...
4. Is the Kill Bill/Kaui server correctly started and listening on port 8080?
5. Are the Kill Bill/Kaui servers accessible from the LB, respectively on port 80 and 9090?
