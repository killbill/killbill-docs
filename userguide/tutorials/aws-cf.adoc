...
Access to these instances is managed by an AWS *Elastic Load Balancer (ELB)*. The ELB routes each request to the correct package and distributes the requests across the available instances. The ELB will listen on port `80` for routing traffic to the Kill Bill instances, and on port `9090` for accessing KAUI.


The elastic load balancer will listen on port `80` for routing traffic to our Kill Bill server instances, and on port `9090` for accessing KAUI, our admin UI. Internally, both Kill Bill server instances, and KAUI server instances will listen on port `8080`. These ports will be different if HTTPS is used (see below).
...
image:https://github.com/killbill/killbill-docs/raw/v3/userguide/assets/img/aws/cf-subscribe.png[align=center]
...
image:https://github.com/killbill/killbill-docs/raw/v3/userguide/assets/img/aws/cf-configure.png[align=center]
...
image:https://github.com/killbill/killbill-docs/raw/v3/userguide/assets/img/aws/cf-step1.png[align=center]
...
image:https://github.com/killbill/killbill-docs/raw/v3/userguide/assets/img/aws/cf-details.png[align=center]
...
image:https://github.com/killbill/killbill-docs/raw/v3/userguide/assets/img/aws/cf-creating.png[align=center]
...
When your stack is complete you can proceed to testing. If you are using HTTPS you should be able to login to KAUI from your browser using the URL https://kaui.DOMAIN, where DOMAIN is *your* domain that you have used for your certificate. If you are using HTTP you will need to use the URL http://DNS_NAME where DNS_NAME is the *DNS name* for the KAUI load balancer as given on the Load Balancer dashboard.
...
== TroubleShooting Section TO BE REVIEWED
