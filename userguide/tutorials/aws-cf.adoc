...
A third alternative available for deploying a production system is CloudFormation. This deployment method leverages the capabilites of the AWS infrastructure to provide a robust **production ready** deployment with a single click. Both Kill Bill and KAUI instances can be scaled up or down; CloudWatch provides metrics to follow what is happening; and the RDS database comes automatically configured and ready for use. The entire CloudFormation configuration, or stack, is defined by a *CloudFormation Template*.
...
The elastic load balancer will listen on port `80` for routing traffic to our Kill Bill server instances, and on port `9090` for accessing KAUI, our admin UI. Internally, both Kill Bill server instances, and KAUI server instances will listen on port `8080`.

The CloudFormation template allows you to specify different groups of subnets to isolate the Kill Bill server and KAUI server instances with the RDS instances. You will need to specify at least two subnets, in different availability zones, for each of those groups. 
...
**Pre-requisites:** The CloudFormation template will require the following:

* A valid VPC
* A VPC Internet Gateway configured to access all the outside world
