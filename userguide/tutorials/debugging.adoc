...
2. Add the following snippet at the entry point of your plugin, e.g. top of `lib/cybersource.rb` for CyberSource:
...
4. Create a Ruby remote debug configuration in RubyMine:
...
5. Restart Kill Bill with the Java property `-X+O`
...
3. Set a breakpoint in the source code using `debugger`
4. Rebuild and redeploy the plugin:
