...
The following list of pre-requisites is needed for this option to work:

1. Setup of the RDS instance with the right Kill Bill schema
2. Setup of the ELB with the correct security groups


Note that we also offer the ability to deploy the whole stack using https://docs.killbill.io/latest//aws-cf.html[CloudFormation templates] to make it easier.

This can be represented with the figure below:

image:https://github.com/killbill/killbill-docs/raw/v3/userguide/assets/aws/standalone_nodb_deployment.png[align=center]

In this model, we rely on AWS ELB to correctly spread the traffic among the various nodes, and also to route traffic to either  Kill Server or KAUI based on the incoming port.

In order to get started, you can follow the same set of instructions as for the One Node Deployment, but instead **TODO AWSMP Link/AMI**

Also when starting the AMIs, you will need to specify the new properties for your RDS instance, and this can be achieved by leveraging the `userdata.sh` script that AWS will run when booting the instance. You can rely on the following script, by simply editing the properties under `EDIT FOLLOWING PROPERTIES BELOW AS NEEDED`:


```
#!/bin/bash

SRC_PROPS="/var/lib/killbill/config/killbill.properties"
DST_PROPS="/var/lib/killbill/config/killbill.properties"

OVERLAY_PROPS="/tmp/overlay.properties"

PROP_MERGE_SCRIPT="/var/lib/tomcat/bin/merge_props.rb"

function write_overlay_props() {
  cat <<_EOF > $OVERLAY_PROPS
#
# EDIT FOLLOWING PROPERTIES BELOW AS NEEDED
# e.g to overwrite dao properties, uncomment following lines
#
#org.killbill.dao.url=jdbc:mysql://mydatabase:3306/killbill
#org.killbill.dao.user=myuser
#org.killbill.dao.password=mypwd
#org.killbill.billing.osgi.dao.url=jdbc:mysql://mydatabase:3306/killbill
#org.killbill.billing.osgi.dao.user=myuser
#org.killbill.billing.osgi.dao.password=mypwd
#
# e.g for a new property, uncomment following lines
...
# Merge existing properties and overlay properties
su -l -c "$PROP_MERGE_SCRIPT $SRC_PROPS $OVERLAY_PROPS $DST_PROPS" tomcat
```

The userdata.sh script can be specified when booting the instance as shown below:
