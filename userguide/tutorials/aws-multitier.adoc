= Installation: Multi-Tier Option

Single AMI deployments are a way to deploy the Kill Bill stack, that is both the Kill Bill server and the administrative UI (KAUI), from one AMI. There are two options: *single-tier* and *multi-tier*. The https://docs.killbill.io/latest//aws-singletier.html[*single-tier* option] is a good way to deploy a limited version of Kill Bill quickly for trial and experimentation. For production deployments, we recommend the *multi-tier* configuration, using two or more EC2 instances and a separate database provided by the AWS *Relational Database Service (RDS)*. This configuration also makes use of an AWS *Elastic Load Balancer (ELB)* to correctly spread the traffic among the various nodes, and to route traffic to either the Kill Bill Server or KAUI based on the incoming port.

As a third option, we also provide an easier way to deploy a production system using https://docs.killbill.io/latest//aws-cf.html[CloudFormation templates]. The multi-tier option requires more setup, but provides more control over the deployment.

This document describes the installation process for the multi-tier option. The diagram below shows the principal components of the multi-tier system: The ELB, the RDS, and two or more EC2 instances. Each EC2 is an instance of Ubuntu Linux, running both Kill Bill and KAUI within a Tomcat server. Clients interact only with the ELB, which routes requests to the rest of the system.
