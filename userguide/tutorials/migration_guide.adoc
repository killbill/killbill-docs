...
Regarding implementation, the forking of the stream can happen in the migration logic proxy. A good way to achieve this result is to write a servlet filter that forwards existing request to old billing system but also call the matching Kill Bill endpoint.

Regarding numbers, one should not expect the same numbers because system will behave differently: Most existing billing system are batch oriented while Kill Bill is event-based, so timing will be very different. Invoicing will also likely vary due to pro-ration logic and potentially different billing alignement. However this is the right time to investigate differences so those are well understood and potentilaly corrected (e.g catalog issue, system configuration issue,..)
