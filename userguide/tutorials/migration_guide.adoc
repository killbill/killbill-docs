...
**Note:** The subscription api have been modified recently to allow specifying both the `entitlementDate` and `billingDate`, and will be fully released as part of `killbill-0.18.x`. Some examples on how to use those apis to achieve the migration goal can be found https://github.com/killbill/killbill/blob/work-for-release-0.18.0/beatrix/src/test/java/org/killbill/billing/beatrix/integration/TestMigrationSubscriptions.java[here].
