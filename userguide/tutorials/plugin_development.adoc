...
Note that when using OSS released plugins, the steps are simpler because the plugin can be installed directly using our http://docs.killbill.io/latest/plugin_management.html#_installation[plugin management apis] and don't require mucking around with copying jars and restarting Kill Bill core.
