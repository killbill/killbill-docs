...
The section explains how you can set up the https://github.com/killbill/killbill-hello-world-java-plugin[hello world plugin]. Once set up, this can be used as the basis for developing your own plugin
...
. Set up the code in Eclipse. You can refer to our https://docs.killbill.io/latest/development.html#_setting_up_code_in_an_ide[development document]  for detailed instructions on how to clone a repository and set up the code in Eclipse
...
If you are using the hello world plugin as a basis to develop your own plugin, it would be useful to deploy the plugin in a standalone Jetty set up. This section explains how you can do this. However, you can also deploy the plugin in a Tomcat or Docker installation of the Kill Bill application. You can also install the Kill Bill application via kpm as specified https://github.com/killbill/killbill-cloud/tree/master/kpm#kill-bill-installation[here] and then install the plugin
...
. Open `PROJECT_ROOT/profiles/killbill/src/main/resources/killbill-server.properties` and specify the following property (Replace `path_to_install_plugin` with the actual path where the plugin is installed. Note that if this property is not specified, the application looks for the plugin at the `/var/tmp/bundles/` path):
[source,properties]
org.killbill.osgi.bundle.install.dir=<path_to_install_plugin>
...
. Ensure that the plugin in deployed in Kill Bill as per the instuctions in the <<Deployment>> section above
...
. Restart the application as explained in *Deployment* section above
