...
|Trigger invoice dry run with target date `2021-10-03`   |Invoice generated for `$25` for `2021-10-03` to `2021-11-03` 
...
|Trigger invoice dry run with target date `2021-10-03`   |Invoice generated with two invoice items corresponding to `standard-monthly` and `oilslick-monthly` plans. Since 
 `OilSlick` uses `START_OF_BUNDLE` alignment rule, its phases are aligned with the phases of the base subscription. So when the `TRIAL` phase of the base subscription ends, the `TRIAL` phase of the `remotecontrol-monthly` subscription ends and an invoice is generated for both subscriptions.
|===
...
* `IMMEDIATE` causess the cancellation to be applied immediately. This type of cancellation rule results in a credit being generated corresponding to the balance of the subscription that they has been paid for but not used.

* The cancellation policy specified here is the *default* cancellation policy. This is applicable only in the case when an explicit cancellation policy is not specified (either via Kaui or via an API call).

[NOTE]
*Note:* 
The cancellation policy specified in this section applies only to _billing_. The cancellation policy for entitlements is  _immediate_ by default as explained https://docs.killbill.io/latest/userguide_subscription.html#components-entitlement[here].
...
|Cancel subscription (Default Policy)| No credit is generated and there is no invoice item corresponding to a credit (Since `BASE` product category uses `END_OF_TERM` cancellation policy)
|===
...
|Cancel addon subscription (Default Policy)| An invoice item is created with a credit of `$15`(Since `ADDON` product category uses `IMMEDIATE` cancellation policy)
|Cancel base subscription (Default Policy)| There is no invoice item corresponding to a credit (Since `BASE` product category uses `END_OF_TERM` cancellation policy)
|===

=== Plan Change Timing

The https://docs.killbill.io/latest/userguide_subscription.html#$$_$$plan_change_timing[plan change timing] rules specify how the system should behave when a plan corresponding to a subscription is changed.

A quick summary of plan cancellation rules:

* A rule specifies one of `phaseType`,  `fromProduct` and `toProduct`, `fromProductCategory` and `toProductCategory`, `fromBillingPeriod` and `toBillingPeriod`, `fromPriceList` and `toPriceList` and its change `policy`.
...
//-------------
|Create a subscription for the `sports-monthly` plan on `2021-09-29` (current date)  |Invoice generated for `$500` for `2021-09-29` to `2021-10-29` 
|Change subscription (Default Policy) to use the `standard-monthly` plan | Plan change takes effect end of term ( No invoice item generated corresponding to a credit) 
