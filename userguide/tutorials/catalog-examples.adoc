...
This rule specify how the phases of an addon subscription align with the phases of the base plan as explained in the https://docs.killbill.io/latest/userguide_subscription.html#$$_$$subscription_alignment_rules[_Subscription Billing_] document. 
...
* *START_OF_BUNDLE* - Causes the phases of the add-on subscription to align with the phases of the base subscription. 
...
This example uses the  https://github.com/killbill/killbill-docs/tree/4671dcd9da1cf021e85629ab67e3ffb6fb553bb1/catalogs/subscription-alignment-plan-creation-addon-phase-alignment.xml[Subscription Alignment Plan Creation Addon Phase] catalog. 
...
It specifies the following addon phase alignment rules:

* If `product=RemoteControl`, alignment=`START_OF_SUBSCRIPTION`
* If `product=OilSlick`, alignment=`START_OF_BUNDLE`
* For anything else, alignment=`START_OF_BUNDLE`
...
|Action   |Result   
//-------------
|Create a subscription corresponding to the `standard-monthly` plan on `2021-09-23` (current date)  |Invoice generated for `$0` as subscription is in trial phase
|Trigger invoice dry run with target date `2021-10-03`   |Invoice generated for `$25` for `2021-10-03` to `2021-11-03` corresponding to  `standard-monthly` plan
...
|Trigger invoice dry run with target date `2021-09-30`   |Invoice generated for `$0` corresponding to the `remotecontrol-monthly` plan as subscription is in trial phase
|Trigger invoice dry run with target date `2021-10-03`   |Invoice generated with only one invoice item corresponding to  `standard-monthly` plan. 
Since `RemoteControl` uses `START_OF_SUBSCRIPTION` alignment rule, its phases are NOT aligned with the phases of the base subscription. So, `remotecontrol-monthly` is still in `TRIAL` phase and does not have an invoice generated.
|===
...
|Trigger invoice dry run with target date `2021-09-30`   |Invoice generated for `$0` corresponding to the `oilslick-monthly` plan as subscription is in trial phase
|Trigger invoice dry run with target date `2021-10-03`   |Invoice generated with two invoice items corresponding to `standard-monthly` and `oilslick-monthly` plans. Since 
 `OilSlick` uses `START_OF_BUNDLE` alignment rule, its phases are aligned with the phases of the base subscription. So when the `TRIAL` phase of the base subscription ends, the `TRIAL` phase of the `remotecontrol-monthly` subscription ends as well and an invoice is generated for both subscriptions.
|===

Note: the default rule (for all other addon products) is `START_OF_BUNDLE`. So, if there was any other addon product, it would be aligned with the base plan.


=== Plan Cancellation Timing

This rule specify how the billing policy is affected when a plan is canceled as explained in the https://docs.killbill.io/latest/userguide_subscription.html#$$_$$plan_cancellation_timing[_Subscription Billing_] document. 

A quick summary of plan cancellation rules:

* A rule specifies one of product category, billing period, price list, phase type and its cancellation policy.

* There are two possible values for cancellation policy, `END_OF_TERM` and `IMMEDIATE`.

* *END_OF_TERM* - Cancellation is applied at the end of the billing period. This type of cancellation rule avoids generating credits.

* *IMMEDIATE* - Cancellation is applied immediately. This type of cancellation rule results in a credit being generated corresponding to the balance of the subscription that they have paid for but not yet used.

* The cancellation policy specified here is the default billing cancellation policy. This can be overrridden with an explicit cancellation policy both via Kaui as well as API calls. 

[NOTE]
*Note:* 
The cancellation policy specified here applies only to _billing_. The cancellation policy for entitlements is always immediate as explained https://docs.killbill.io/latest/userguide_subscription.html#components-invoicing[here].

This example uses the  https://github.com/killbill/killbill-docs/blob/1a945188c0ef53a3c7f797b4890514d35c09c46f/catalogs/subscription-alignment-plan-cancellation-timing.xml[Subscription Alignment Plan Cancellation Timing] catalog. 
...
* If `productCategory=BASE`, cancellation policy=`END_OF_TERM`
* If `productCategory=ADD_ON`, cancellation policy=`IMMEDIATE`
* For anything else, cancellation policy=`END_OF_TERM`
...
|Action   |Result   
//-------------
|Create a subscription for the `standard-monthly` plan on `2021-09-29` (current date)  |Invoice generated for `$25` for `2021-09-29` to `2021-10-29` corresponding to  `standard-monthly` plan
|Cancel subscription (Default Policy)| Since `BASE` product category uses `END_OF_TERM` cancellation policy, no credit is generated. So, there is no invoice item corresponding to a credit.
...
|Cancel addon subscription (Default Policy)| Since `ADDON` product category uses `IMMEDIATE` cancellation policy, an invoice item is created with a credit of `$15`
|Cancel base subscription (Default Policy)| Since `BASE` product category uses `END_OF_TERM` cancellation policy, credit is generated.
|===

TODO: add about default rule?

=== Plan Change Timing

This rule specifies how the system should behave when a plan corresponding to a subscription is changed as explained in the https://docs.killbill.io/latest/userguide_subscription.html#$$_$$plan_change_timing[_Subscription Billing_] document.

A quick summary of plan cancellation rules:

* A rule specifies one of either phaseType or  fromProduct/toProduct or fromProductCategory/toProductCategory or fromBillingPeriod/toBillingPeriod or fromPriceList/toPriceList and its change policy.

* There are three possible values for change policy, `END_OF_TERM`,  `IMMEDIATE` and `ILLEGAL`.

* END_OF_TERM - specifies that the change should happen at the end of the current billing period.

* IMMEDIATE - specifies that the change should happen immediately.

* ILLEGAL - specifies that the plan change is not allowed.

This example uses the  https://github.com/killbill/killbill-docs/blob/1a945188c0ef53a3c7f797b4890514d35c09c46f/catalogs/subscription-alignment-plan-change-timing.xml[Subscription Alignment Plan Change Timing] catalog. 
...
. If `phaseType=TRIAL`, change policy=`IMMEDIATE`
. If `fromProduct=Sports` and `toProduct=Super`, change policy=`IMMEDIATE`
. If `toProduct=Premium`, change policy=`IMMEDIATE` (any `fromProduct` value)
. If `fromProduct=Premium` and `toProduct=Standard`, change policy=`ILLEGAL`
. For anything else, change policy=`END_OF_TERM`
...
|Action   |Result   
//-------------
|Create a subscription for the `standard-monthly` plan on `2021-09-29` (current date)  |Invoice generated for `$0` due to trial phase
|Change subscription (Default Policy) to use the `sports-monthly` plan | Plan change takes effect immediately (Invoice generated for `$500` for `2021-09-29` to `2021-10-29`) 
|Trigger invoice dry run with target date `2021-10-29`    |Invoice generated for  `$500` for `2021-10-29` to `2021-11-29`  
|===
...
|Change subscription (Default Policy) to use the `super-monthly` plan | Plan change takes effect immediately (Invoice generated for `$500` for `2021-09-29` to `2021-10-29` for the balance amount)
|Trigger invoice dry run with target date `2021-10-29`    |Invoice generated for  `$1000` for `2021-10-29` to `2021-11-29`  
|===
...
//-------------
|Create a subscription for the `sports-monthly` plan on `2021-09-29` (current date)  |Invoice generated for `$500` for `2021-09-29` to `2021-10-29` 
|Change subscription (Default Policy) to use the `premium-monthly` plan | Plan change takes effect immediately (Invoice generated for `$1500` for `2021-09-29` to `2021-10-29` for the balance amount)
|Trigger invoice dry run with target date `2021-10-29`    |Invoice generated for  `$2000` for `2021-10-29` to `2021-11-29`  
|===
...
|Change subscription (Default Policy) to use the `standard-monthly` plan | Plan change not allowed
|===
...
//-------------
|Create a subscription for the `sports-monthly` plan on `2021-09-29` (current date)  |Invoice generated for `$500` for `2021-09-29` to `2021-10-29` 
|Change subscription (Default Policy) to use the `standard-monthly` plan | Plan change takes effect end of term so no credit generated 
|Trigger invoice dry run with target date `2021-10-29`    |Invoice generated for  `$100` for `2021-10-29` to `2021-11-29`  
|===
