...
|autoReevaluation + 
Interval   |Tag within `state`   |Specifies the duration after which the overdue state should be reevaluated. The duration needs to be specified with a `unit` (one of `DAYS`, `WEEKS`, `MONTHS`, `YEARS`, `UNLIMITED`) and an integer value. |                 <autoReevaluationInterval>
                 <unit>DAYS</unit>
                 <number>4</number>
                </autoReevaluationInterval>
