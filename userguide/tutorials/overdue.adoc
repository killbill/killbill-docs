...
Having said that, most business cases would require using both the systems in conjunction with one another. Even the scenario defined above requires attempting the payment 4 times as well as taking some actions when the balance is unpaid. Thus, both the systems come into play.

Although the payment retry system and overdue systems need to be configured independently of each other, it makes sense to *align payment retries with overdue states*. This allows retrying the payment on the boundary of each overdue state and thus prevents transitioning from one state to the next if the payment gets retried successfully. In the scenario defined above, the 2nd payment attempt is after 9 days (just before the first overdue state), the 3rd payment attempt is after 13 days (before the second overdue state) and the final payment attempt is after 20 days ( before the last overdue state).  
...
. As you proceed with the steps below, you can verify each step by viewing the account in Kaui. The main account screen includes an `OVERDUE STATUS` field in the `Billing Info` section. The `Timeline` tab within the account screen includes information about payment retries. The `payment` and `invoices` tab include information about payments and invoices. 
...
[start = 15]
. Move clock +5 day (`2021-09-26`) and observe the final payment retry. Verify that the payment is successful and the account is moved to the `CLEAR` status.
