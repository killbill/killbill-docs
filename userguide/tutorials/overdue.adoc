...
In order to test the system, one must be able to make payment fail. For that purpose, we created a https://github.com/killbill/killbill-payment-test-plugin[payment test plugin] that be be configured through api to make payments fail. 
...
-X POST \
--data-binary '{"name":"Arthur","email":"arthur@laposte.fr","externalKey":"arthur","currency":"EUR"}' \
...
[start = 3]
3. Create a subscription in the past to trigger some invoices and (failed) payment. 

Obviously the ids, catalog, and dates used below ashould be adjusted with valid values and to achieve the correct overdue state
...
-H "X-Killbill-CreatedBy: demo" \
-X POST \
--data-binary '{"accountId":"60035793-cbe5-472a-8bd8-3c67cc3beaf4","externalKey":"arthur9","productName":"Standard","productCategory":"BASE","billingPeriod":"MONTHLY","priceList":"DEFAULT"}' \
"http://127.0.0.1:8080/1.0/kb/subscriptions?requestedDate=2015-09-01"
----
