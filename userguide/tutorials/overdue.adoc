...
Kill Bill has two distinct built-in mechanisms that come into play here, these are the *Payment Retry System* and the *Overdue System*. Though separate, these two systems work in close conjunction with each other.
...
Having said that, most business cases would require using both the systems in conjunction with one another. Even the scenario defined above requires attempting the payment 4 times as well as blocking the entitlements and changing the user's access when the balance is unpaid. Thus, both the systems come into play.

Although the payment retry system and overdue systems need to be configured independently of each other, it makes sense to *align payment retries with overdue states*. This allows retrying the payment on the boundary of each overdue state and thus prevents transitioning from one state to the next if the payment gets retried successfully. In the scenario defined above, the 3rd payment attempt is after 10 days (just before the first overdue state) and the final payment attempt is after 16 days ( before the last overdue state).  Ideally, it would make sense to configure another payment retry just before the second overdue state as well.
...
[start = 5]
. Create a subscription and verify in the Kaui *Invoices* tab that a $0 invoice is generated:
...
[start = 6]
. Move the clock to reach end of trial (`2021-08-06`) and see first payment in the *Payments* tab in Kaui:
...
[start = 8]
. Move the clock to the next month (`2021-09-06`) and observe first failed payment in the *Payments* tab:
...
[start = 9]
. Move clock +1 day (`2021-09-07`) and observe first payment retry in the *Timeline* tab:
...
[start = 10]
. Move clock +1 day (`2021-09-08`) and observe second payment retry in the *Timeline* tab:

. Move clock +8 day (`2021-09-16`) and observe third payment retry in the *Timeline* tab. Also verify that the account is in `WARNING` status in the main *Account* tab:

. Move clock +4 day (`2021-09-20`) and verify that the account is in `BLOCKED` status in the *Account* tab:
...
[start = 14]
. Move clock +1 day (`2021-09-22`) and observe the final payment retry in the *Timeline* tab. Verify that the payment is successful and the account is moved to the `CLEAR` status in the *Account* tab:
...
=== What happens if the payment retry schedule and overdue XML is not configured?
