...
Kill Bill has two distinct built-in mechanisms that come into play in such situations, these are the *Payment Retry System* and the *Overdue System*. Though separate, these two systems work in close conjunction with each other.
...
Having said that, most business cases would require using both the systems in conjunction with one another. This is because most business cases would prefer to retry payments several times before blocking the user's access.
...
Note that in the scenario defined above, the 2nd payment attempt is after 9 days (just before the first overdue state), the third payment attempt is after 13 days (before the second overdue state) and the final payment attempt is after 20 days ( before the last overdue state). Thus, we are configuring the system to retry payments 1 day before each overdue state.
