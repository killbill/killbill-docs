...
[source, xml]
<state name="WARNING">  
|condition     |Tag within `state` |Condition to be evaluated. See <<Overdue Conditions>>  | See <<Overdue Conditions>>
...
|subscriptionCancellationPolicy   |Tag within `state`|Specifies whether or not to cancel the subscriptions associated with the account (and the cancellation policy that should be used) when this overdue state is reached. Possible values are `NONE`, `IMMEDIATE`, `END_OF_TERM`)     |
[source, xml]
...
|disableEntitlementAndChangesBlocked  |Tag within `state`|Specifies whether the subscriptions should be paused (which means entitlement and billing will be disabled) when this overdue state is reached. A true/false value needs to be specified.    |
[source, xml]
<disableEntitlementAndChangesBlocked>
true
</disableEntitlementAndChangesBlocked>
|autoReevaluationInterval   |Tag within `state`   |Specifies the duration after which the overdue state should be reevaluated. The duration needs to be specified with a `unit` (one of `DAYS`, `WEEKS`, `MONTHS`, `YEARS`, `UNLIMITED`) and an integer value |  [source, xml]
                <autoReevaluationInterval>
                 <unit>DAYS</unit>
                 <number>4</number>
                </autoReevaluationInterval>
...
|enterStateEmailNotification   |Tag within `state`   |Deprecated|
NA
|===

=== Overdue Conditions

The following table lists the various overdue conditions. Note that it is also possible to combine multiple conditions in which case the overall condition will evaluate to `TRUE` only if *ALL* the specified conditions evaluate to `TRUE`.

[cols="a,a,a"]
|===
|*Condition*   |*Description* |*Example*
//-------------
|numberOfUnpaidInvoicesEqualsOrExceeds   |Specifies that the overdue flow should kick in when the number of unpaid invoices equals or exceeds the specified value. An integer value needs to be specified.|  
[source, xml]
<condition>
<numberOfUnpaidInvoicesEqualsOrExceeds>2
</numberOfUnpaidInvoicesEqualsOrExceeds>
</condition>
|totalUnpaidInvoiceBalanceEqualsOrExceeds   | Specifies that the overdue flow should kick in when the total unpaid invoice balance equals or exceeds the specified value. A decimal value needs to be specified.  |
[source, xml]
<condition> 
  <totalUnpaidInvoiceBalanceEqualsOrExceeds>1.00
   </totalUnpaidInvoiceBalanceEqualsOrExceeds>
</condition> 
|timeSinceEarliestUnpaidInvoiceEqualsOrExceeds   |Specifies that the overdue flow should kick in when the specified duration has elapsed since the last unpaid invoice. The duration needs to be specified with a `unit` (one of `DAYS`, `WEEKS`, `MONTHS`, `YEARS`, `UNLIMITED`) and an integer value  | 
[source, xml]
<condition>
  <timeSinceEarliestUnpaidInvoiceEqualsOrExceeds>
    <unit>DAYS</unit>
...
|responseForLastFailedPaymentIn|Specifies that the overdue flow should kick in when payment fails and the failed response is one of the specified values.  Possible values are defined in https://github.com/killbill/killbill-api/blob/4ae1c343a593de937415e21feecb9f5405037fa3/src/main/java/org/killbill/billing/payment/api/PaymentResponse.java[PaymentResponse]. Multiple values may be specified.    |   
[source, xml]
<condition>
  <responseForLastFailedPaymentIn>
    <response>INVALID_CARD</response>
    <response>LOST_OR_STOLEN_CARD</response>
     <response>INSUFFICIENT_FUNDS</response>
  </responseForLastFailedPaymentIn>
</condition>
|controlTagInclusion   |Specifies that the overdue flow should kick in when the account is tagged with the specified control tag. Note that this condition cannot be specified by itself and can only be specified along with one of the other conditions as shown. | 
[source, xml]
<condition>
  <totalUnpaidInvoiceBalanceEqualsOrExceeds>5.00
  </totalUnpaidInvoiceBalanceEqualsOrExceeds>
  <controlTagInclusion>TEST
  </controlTagInclusion>
</condition>
|controlTagExclusion   |Specifies that the overdue flow should kick in when the account is NOT tagged with the specified control tag. Note that this condition cannot be specified by itself and can only be specified along with one of the other conditions.  |
[source, xml]
<condition>
  <totalUnpaidInvoiceBalanceEqualsOrExceeds>5.00
  </totalUnpaidInvoiceBalanceEqualsOrExceeds>
...
* https://github.com/killbill/killbill-docs/blob/4671dcd9da1cf021e85629ab67e3ffb6fb553bb1/overdue-configurations/overdueWithNumberOfUnpaidInvoicesCondition.xml[overdueWithNumberOfUnpaidInvoicesCondition.xml]

* https://github.com/killbill/killbill-docs/blob/4671dcd9da1cf021e85629ab67e3ffb6fb553bb1/overdue-configurations/overdueWithTotalUnpaidInvoiceBalanceCondition.xml[overdueWithTotalUnpaidInvoiceBalanceCondition.xml]

* https://github.com/killbill/killbill-docs/blob/4671dcd9da1cf021e85629ab67e3ffb6fb553bb1/overdue-configurations/overdueWithControlTag.xml[overdueWithControlTag.xml]

* https://github.com/killbill/killbill-docs/blob/4671dcd9da1cf021e85629ab67e3ffb6fb553bb1/overdue-configurations/overdueWithExclusionControlTag.xml[overdueWithExclusionControlTag.xml]

* https://github.com/killbill/killbill-docs/blob/4671dcd9da1cf021e85629ab67e3ffb6fb553bb1/overdue-configurations/overdueWithMultipleConditions.xml[overdueWithMultipleConditions.xml]

== Overdue.xml validation

The overdue.xml file can be validated using the overdue.xsd file provided https://docs.killbill.io/latest/overdue.xsd[here].
