...
Although the payment retry system and overdue systems need to be configured independently of each other, it makes sense to *align payment retries with overdue states*. This allows retrying the payment on the boundary of each overdue state. Thus, this prevents transitioning from one state to the next if the payment gets retried successfully. In the scenario defined above, the 3rd payment attempt is after 10 days (just before the first overdue state) and the final payment attempt is after 16 days ( before the last overdue state).  Ideally, it would make sense to configure another payment retry just before the second overdue state as well.
