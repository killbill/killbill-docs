...
* `actionType` : Specifies whether the subscription is `EFFECTIVE` (subscription created with the current date) or `REQUESTED` (subscription created with a future date). 
