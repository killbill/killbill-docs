...
Now proceed with the rest of the setup as described under single tier. However, there is one important difference. When you edit the security group for your instances (all should use the same one), you will need to identify the ports to be used by the load balancer in place of those expected by the external client. Thus, the inbound rules for your security group should now look like this:

image:https://github.com/killbill/killbill-docs/raw/v3/userguide/assets/aws/multitier-inbound.png[align=center]
...
If you have selected the HTTPS protocol, you will now be required to create or provide an X.509 SSL Certificate. There are many ways to do this. Some are discussed below. If you are using HTTP, continue here.
