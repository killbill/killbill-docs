...
The last major task is to setup the Elastic Load Balancer in front of the EC2 instances. You will actually need to deploy two separate load balancers, one for access to KAUI, and one for the KB server. We will describe the procedure for one ELB, and you will need to repeat this for each one.
...
3. Set the listener port to 443 (for the KAUI load balancer) or 8443 (for the Kill Bill load balancer).
4. Scroll to the bottom and select at least *two* availability zones. *IMPORTANT: You must select all of the zones that your EC2 instances are contained in. Otherwise, the load balancer will be unable to connect to these instances.*
...
If you have selected the HTTPS protocol, you will now be required to create or provide an X.509 SSL Certificate. There are many ways to do this. Some are discussed below. If you are using HTTP, you will see a warning message. Ignore the message and choose *Next: Configure Security Groups*. This will take you to a page titled *Step 3: Configure Security Groups.*  This page will show you the existing security groups and offer the choice to choose an existing group or create a new one. We advise you to create a new group, which will then be used by both load balancers. The new security group should have inbound rules enabling inputs for KAUI and Kill Bill as shown here: 

image:https://github.com/killbill/killbill-docs/raw/v3/userguide/assets/aws/ELB-configure-security-groups.png[align=center]

The next step is to identify the *target* instances for each load balancer, which are collected into a *target group*. For each ELB you will need to create a new target group. Each group will consist of all of the instances you have launched. First, create the group and give it a simple name:

image:https://github.com/killbill/killbill-docs/raw/v3/userguide/assets/aws/ELB-configure-routing.png[align=center]

On this page the port must be set to 3000 for the KAUI ELB, or to 8080 for the Kill Bill ELB.

Next scroll to the bottom of the same page and expand the section named *Advanced health check settings*. In the field named *Success Codes* enter "200,302".

image:https://github.com/killbill/killbill-docs/raw/v3/userguide/assets/aws/ELB-healthcheck.png[align=center]


Now click on *Next: Register Targets*. You are nearly done. The purpose of this final step is to identify the target instances that will be part of your target group. Initially all your instances will be listed in the bottom section. To register them, select them all and click *Add to Registered*. Then proceed to *Next: Review*.
...
When both ELBs are ready you can proceed to testing. You should be able to login to KAUI from your browser using the URL https://DNS_NAME:443/users/sign_in, where DNS_NAME is the *DNS name* for the KAUI load balancer as given on the Load Balancer dashboard. This should display the KAUI login screen. The default credentials are: *admin/password*. The first few requests might be a bit slow as Kill Bill initializes itself.

For an introduction to KAUI, see our http://docs.killbill.io/latest/getting_started.html#_using_kill_bill_with_kaui[Getting Started] guide.

Similarly, you should be able to login directly to the Kill Bill server using the URL https://DNS_NAME:8443.
