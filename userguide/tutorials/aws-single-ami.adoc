...
In addition, AWS places all resources within a *Virtual Private Network (VPC)*. A default VPC will be created and used automatically in the following steps. However, if you have access to other VPCs, you will need to ensure that all Kill Bill respources are deployed in the same one.
...
If you have selected the HTTPS protocol, you will be required to create or provide an X.509 SSL Certificate. If you have a certificate you can identify or upload it here. Otherwise we recommend you create a new one using the *Amazon Certificate Manager* as discussed below.
...
image:https://github.com/killbill/killbill-docs/raw/v3/userguide/assets/aws/ELB-rules.png[align=center]

Your objet now is to change the rule for this listener to point to your new target group. To do this:
