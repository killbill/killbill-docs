...
The account for which emails need to be sent needs to have the `email` and `locale` fields set. In addition, the default templates also require the  `company`, `address1`, `city`, `state`, `postalCode`, `country` fields also set on the Account.
