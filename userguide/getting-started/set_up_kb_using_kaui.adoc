...
You will be prompted for a username and password. The default set of credentials is `admin`/`password`, which grants full access.

[NOTE]
*Note: Both Kill Bill and Kaui support role based access control (RBAC), where you can configure fine-grained permissions for your users. 
...
For more information on tenants, see the following: 
